...
> **Not purely "more corpus" — the change is "more corpus AND shorter chunks".** A note is ~50 words against `CHUNK_WINDOW_WORDS` = 800, so each note becomes a single short chunk, and short chunks score higher against short titles. Notes displace longer excerpts by construction. Which half of the mechanism carries the gain is **not separated**, and separating it means re-chunking, which invalidates every other column in the results table. Same shape of confound as arm 2 vs arm 3's excerpt volume — recorded, not resolved.
