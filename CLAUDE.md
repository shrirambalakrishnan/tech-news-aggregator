...
5. **`voyageapi` package** (Approach 3): thin Voyage AI client, mirroring `claudeapi`, covering two endpoints. **Embeddings** (`POST /v1/embeddings`): `EmbedDocuments([]string) ([][]float32, error)` token-batches inputs and **paces requests for Voyage's no-payment tier** (3 RPM / 10K TPM): token-bounded batches, an inter-request delay, and 429 retry-with-backoff (all tunable package vars). **Rerank** (`POST /v1/rerank`, Approach 6): `Rerank`/`RerankMany` score (query, document) pairs with `VOYAGE_RERANK_MODEL` = `rerank-2.5`; `RerankMany` is arm 4's entrypoint and paces between calls (~52s at 6 chunks) — a cross-encoder scores one query per call, so N stories cost N requests and cannot be batched. Reads `VOYAGE_API_KEY`. Organized **one file per layer**, with the second endpoint spread across the same layers rather than forming a parallel stack: `voyage.go` (PUBLIC API: `EmbedDocuments` + the `embedBatch` DI seam), `rerank.go` (PUBLIC API: `Rerank`/`RerankMany` + the `rerankBatch` seam), `ratelimit.go` (POLICY: free-tier config vars, `planBatches`, `pacingDelay`, `pace`, retry), `client.go` (TRANSPORT: wire types + `embedBatchHTTP`/`rerankHTTP`).

   > **One rate-limit policy covers both endpoints — the throttle is the ACCOUNT's, not the endpoint's.** 3 RPM / 10K TPM was measured on embeddings and confirmed by probe on `/v1/rerank` at the shape arm 4 sends (6 chunks + 1 title), where the paced ~52s behaved as predicted. So there is no parallel `VOYAGE_RERANK_*` constant set: both paths share `VOYAGE_TPM_LIMIT`, `VOYAGE_MIN_REQUEST_GAP`, `VOYAGE_MAX_RETRIES` and `VOYAGE_RETRY_BACKOFF` (60s, linear: 1min, 2min, 3min…), and both wait through `pace`, which logs the delay and the two numbers behind it before sleeping — a paced run is otherwise minutes of silence after a line describing a request that already succeeded, i.e. indistinguishable from a hang. The only per-endpoint difference is the **token count** handed to `pacingDelay`: a batch's estimated tokens for embeddings, the fixed `voyageapi.RERANK_TOKENS_PER_CALL` (7859, measured) for rerank, whose request shape does not vary.
...
size. `RETRIEVAL_SIMILARITY_FLOOR` shipped at 0.0 and inert and is now the
**measured 0.3330** (issue #25) — see Approach 4 below for what its inert period
cost. `TopScoresPerQuery(queries, k)` exposes the raw scores for calibration
without exporting `cosineSimilarity`. `rerank.go` is arm 4's retrieval stage
(Approach 6): `RetrieveRerankedContext(queries)` runs the same per-story cosine
selection, widened to `RERANK_CANDIDATES_PER_STORY` (6) at the permissive
`RERANK_CANDIDATE_FLOOR` (0.0, **inert in practice** — see Approach 6), hands each
story's candidates to the cross-encoder via the `rerankQueries` DI seam, keeps
...
- **`evalRuns/<run_id>.json`** (`evalHarness/runrecord.go`) — `run_id`, `arm`, `git_sha`, `dataset_hash`, `corpus_index_hash`, `model`, `rerank_model`, `config`, `metrics{tp,fp,tn,fn,precision,recall}`. **`config` (issue #40) names the arm's tuning constants and their values**, built by `configForArm` — a third sibling of `armUsesCorpusIndex`/`armUsesReranker` carrying the same obligation to mirror `armcontext.BuildProfile`'s cases. `eval_batch_size` for every arm, plus `retrieval_top_k` (arm 2), `retrieval_top_k_per_story`/`retrieval_similarity_floor`/`retrieval_pool_cap` (arm 3), and `rerank_candidates_per_story`/`rerank_candidate_floor`/`rerank_top_k_per_story`/`retrieval_pool_cap` (arm 4). Arm 4 records neither the similarity floor nor `retrieval_top_k_per_story`: it selects candidates by **rank**, so recording them would claim an input it never consulted. Values are formatted to string **once, at write time** — the map is the canonical form that gets hashed, so re-deriving `"0.3330"` as `"0.333"` elsewhere would split a group for a formatting reason. `rerank_model` stays a **root-level field, not a config entry**: it is a model identity like `model` — a value you read — and burying it in a hash would undo issue #39. `corpus_index_hash` uses `omitempty` and is **absent for arms 0 and 1**, which never load the index — an empty string would read as "used an empty index" rather than "did not use one". `rerank_model` (issue #39) is `omitempty` for the same reason and is **absent for arms 0–3**, which never rerank. It is recorded even though `voyageapi.VOYAGE_RERANK_MODEL` is a compile-time `const` that `git_sha` already pins: it earns its place the way `model` does — a reader can see which cross-encoder ranked the excerpts without checking out the commit, and the grouping key stops implying the reranker was held constant.
...
- **`armUsesCorpusIndex` mirrors `armcontext.BuildProfile`'s retrieval cases** and must be updated alongside them. An arm that reads the index but is missing there records no `corpus_index_hash` and publishes numbers whose retrieval side is untraceable. Arm 4 was added to it with the arm itself, for exactly that reason. **`armUsesReranker` (in `runrecord.go`) is its sibling** and carries the same obligation for `rerank_model`. It lives apart from `armUsesCorpusIndex` deliberately: nothing is archived and no file is hashed for it — it gates a struct field, not file I/O.
...
> **Known limitations, all deliberate.** (1) **`git_sha` is HEAD, not the working tree** — an uncommitted edit is invisible, so a record can name a commit whose code is not what ran. *Issue #40 narrows this: the constants in `config` are read from the running process, so an uncommitted tuning change is now recorded correctly. What remains uncovered is code `config` does not name — prompt wording above all.* (2) **Arm 1's `profile/user_context.json` is NOT hashed.** It is equally git-ignored, equally regenerable, and equally decides arm 1's numbers, so arm 1 records pin the code and model but not the profile actually used — an asymmetry with arms 2/3, left as a follow-up rather than an oversight. (3) `rag.BuildCorpusIndex` fails soft (logs, returns nothing), so `main.runEmbed` cannot tell a successful build from a failed one and will archive whichever index is on disk. Truthful about what the RAG arms would read *now*, but not evidence this run produced it; making `BuildCorpusIndex` return an error would close the gap and was deliberately not bundled in.

> **Closed by issue #39: `rerank_model` IS on the record**, populated from `voyageapi.VOYAGE_RERANK_MODEL` for arm 4 only (`armUsesReranker`), `omitempty` everywhere else. It landed before the second reranker was tried, which was the point — records cannot be back-filled, so every arm-4 record written before it is permanently unattributable on the ranking side. **One such record exists** (`20260809T094948Z-arm-4`, the first arm-4 run): it was measured under `rerank-2.5`, and `eval-report` renders its rerank model as `—` because the file does not say so. Treat that `—` on an arm-4 row as "written before #39", not as "did not rerank".
...
- **Output 1 — one row per run**, sorted by `run_id` (lexically chronological by construction): `run_id, arm, config(12), git_sha(7), dataset_hash(12), corpus_index_hash(12), model, rerank_model, TP, FP, TN, FN, precision, recall`. It is the **only** output still carrying `git_sha`, which after issue #40 is provenance rather than identity — a group can span commits, so a sha beside a group would name one arbitrary member as though it spoke for all of them. Hashes are abbreviated; **neither model is** — both are grouping keys, and truncating one could make two different models look like one configuration (`shortHash` would render `rerank-2.5-lite` as `rerank-2.5-l` — a model id Voyage does not have, with the suffix separating it from `rerank-2.5` cut off — which is why `orAbsent` renders this column). An absent `corpus_index_hash` (arms 0/1) or `rerank_model` (arms 0–3) renders `—`, never blank or `0`.
- **Output 2 — one row per configuration**, grouped by `(arm, config, model, rerank_model, dataset_hash, corpus_index_hash)` and sorted by `n` desc then every key field asc — `sortGroups` compares every key field, so adding one to the key without adding it there reintroduces the nondeterminism. The tiebreak is load-bearing: Go randomises map iteration, so without a total ordering the report is nondeterministic and untestable. Counts are means; precision/recall carry mean and **sample** stddev (n−1 — the runs are draws from a non-deterministic process, not a complete population).
- **The grouping key is the config, not the commit (issue #40).** `git_sha` pinned the tuning constants by pinning the whole commit, which over-pinned them: any commit at all changed the sha, so two runs with identical tuning stopped grouping as repeats the moment an unrelated commit landed between them — five arm-2 records on disk are split exactly this way, across two shas whose only code difference is this aggregator. `fingerprint(record)` is `sha256(json.Marshal(config))`, and it is canonical **only because `encoding/json` sorts map keys** — a hand-rolled `range`-and-concat would compile, look equivalent, and randomise. It is **derived at read time, never stored**: a fingerprint written into the record would pin the canonicalisation of the day it was written, so changing that later would give two identical configs two different fingerprints — the same bug one layer down. A record with **no `config`** (written before #40, and unbackfillable) falls back to its `git_sha`, so legacy records group exactly as they did before; defaulting them to an empty config would merge them on the claim that they shared constants nobody recorded.
- **A `=== N configurations ===` legend** expands every fingerprint into its `key=value` list, sorted by key. The tables show a hash because a column per constant would grow with every arm and every knob — the thing #40 set out to avoid — but a hash alone would leave the report *less* readable than the `git_sha` it replaced, which could at least be pasted into `git show`. Pre-config entries render `(pre-config record — grouped by git_sha)` rather than as an empty config.
...
**Limitations inherited from the record and undetectable here:** `config` names the listed tuning constants only, so a code change outside them — prompt wording above all — no longer splits groups now that `git_sha` has left the key; and arm 1's `profile/user_context.json` is not hashed, so arm-1 rows group on a key omitting an input that decides their numbers. Both are printed as a footnote under Output 2.
...
> ⚠️ **Voyage pacing gap in the arm 2 eval (known, unfixed).** `EmbedQuery` has 429 retry-with-backoff but — unlike `EmbedDocuments` — applies **no** `VOYAGE_MIN_REQUEST_GAP` pacing between calls, because production only ever makes *one* retrieval call per run. The eval makes **12 back-to-back**, which on the free tier (3 RPM) means calls 4+ get 429'd and self-pace via linear backoff (60s, 120s, …, `VOYAGE_MAX_RETRIES` = 6 — `VOYAGE_RETRY_BACKOFF` was doubled from 30s in Approach 6, since the first 30s retry almost always failed). It completes, but takes several minutes with wasted round trips. Fix when it becomes annoying: either sleep `VOYAGE_MIN_REQUEST_GAP` between eval retrievals, or move min-gap pacing into `voyageapi` so it applies to queries too. Unaffected if a payment method is on the Voyage account.
...
### The decisions taken while planning (three of them changed the spec)
...
2. **Pacing ships at the spec's ~52s, and the throttle behind it is measured.**
   ...
   embeddings constants rather than getting a parallel set — same limits, one
   place to change when the account tier does. `VOYAGE_RETRY_BACKOFF` was also
   doubled 30s → 60s **on the shared constant**, since the first 30s retry almost
   always fails; that intentionally slows the embed path's backoff too.
   > ⚠️ **"One call fits inside 10K tokens" does NOT remove the need to pace.**
   > TPM is a rate. At the 3 RPM floor alone, three calls a minute would offer
   > `7,859 × 3 = 23,577` tokens against a 10K budget — near-continuous 429s
   > backing off 1min/2min/3min, i.e. **slower than simply waiting**. So
   > `RerankMany` keeps `pacingDelay(RERANK_TOKENS_PER_CALL)` ≈ 52s, routed
   > through `pacingDelay` rather than hardcoded, so lifting `VOYAGE_TPM_LIMIT`
   > when a payment method is added speeds up both endpoints with no code change.
...
raising it raises the pacing delay roughly linearly **and requires raising
`voyageapi.RERANK_TOKENS_PER_CALL` with it**, since under-counting there buys
429s rather than throughput). `RERANK_TOP_K_PER_STORY` = 2, deliberately equal to
`RETRIEVAL_TOP_K_PER_STORY` so the pooled volume stays comparable to arm 3's.
...
>
> **Inert IN PRACTICE, not in principle — a correction the tests forced.** The
> plan asserted the floor "cannot fire". It can: `topKAboveFloor` drops scores
> *strictly below* the floor, so 0.0 would filter a **negative** cosine. There
> simply are none — embeddings sit in a positive cone and the observed range over
> this corpus is ~0.09–0.51 — which is why it is inert here and would not be on a
> corpus that produced negative scores. **Design lesson: "this knob cannot fire"
> is a claim about the data as much as the code; write which one it rests on.**
...
- ~~**`rerank_model` on the run record.**~~ **Shipped in issue #39**, before any
  second reranker was tried — which was the deadline that mattered, since records
  cannot be back-filled. See the eval-pipeline section for what it pins and for
  the one arm-4 record that predates it.
...
- **Eval-report presentation:** no cross-arm ranking, no README
  auto-update. PR #23's category.
