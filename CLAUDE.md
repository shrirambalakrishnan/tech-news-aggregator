...
index; live in **both** the classify path and `evalHarness`). Both `main`
...
returns an empty profile: its `RetrievedExcerpts` depend on the batch's titles (the
retrieval query), which only `FilterHackerNewsStoriesByTitle` (production) and
`classifyInBatches` (eval) have, so retrieval — and the hard error on a missing
index — happens there instead. Both build the query identically: the batch's
titles joined by newline, so the eval measures production's retrieval shape.
...
**Eval arm (done):** `go run . eval 2` works. `evalHarness.classifyInBatches` re-runs retrieval **per batch**, keyed by that batch's own titles, so each of the 12 batches gets its own top-k chunks — the same retrieval shape production uses, rather than one global context reused across the dataset. A retrieval failure aborts the eval (no fail-soft), for the same reason as the classify path.
