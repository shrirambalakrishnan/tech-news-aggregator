...
func TestConstructPromptSystemAttribute(t *testing.T) {
	...
	// every arm, so it says nothing about how many excerpts each arm actually
	// supplies. Arm 2's volume differs from arms 3 and 4 (5 vs up to 20) and is
	// pinned separately by rag.TestRetrievalArmsInjectDifferentExcerptVolumes;
	// arms 3 and 4 are volume-matched to each other by their equal per-story k.
